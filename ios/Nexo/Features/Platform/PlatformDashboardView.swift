...
private struct GymsListView: View {
    ...
    var body: some View {
        if gyms.isEmpty {
            ...
            .sheet(item: $gymToEdit) { gym in
                GymSettingsSheet(gym: gym) { updatedGym in
            ...
            .alert(
                "Delete Gym?",
                isPresented: Binding(get: { gymToDelete != nil }, set: { if !$0 { gymToDelete = nil } })
            ) {
                Button("Delete", role: .destructive) {
                    if let gym = gymToDelete { Task { await deleteGym(gym) } }
                }
                Button("Cancel", role: .cancel) { gymToDelete = nil }
            } message: {
                Text("Are you sure you want to delete \(gymToDelete?.name ?? "this gym")? This will permanently delete all classes, bookings, plans, memberships, and records. This action cannot be undone.")
            }
            .alert(
                "Error",
    ...
    private func deleteGym(_ gym: Gym) async {
        do {
            try await FirebaseBackend.shared.deleteGym(gymId: gym.id)
            gyms.removeAll { $0.id == gym.id }
            appState.myGyms.removeAll { $0.gym.id == gym.id }
